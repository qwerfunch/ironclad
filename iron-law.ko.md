...
- `stage_4.1 Audit` 은 AC 별 2 중: (a) evidence 충분성, (b) 코드/테스트 검토. 같은 사람이 둘 다 sign — AI 자가검증 차단.
...
4 레벨 모두 통과해도 최대 LLM 호출 1 회. detector graph 의 constant-cost pruning 과 결합하면 *피처당 총 LLM 비용 O(1)*.
...
| (없음) | **L4 (HITL)** | **진짜 새 발명 — AI 자가검증 차단** |

정직한 명명: L1-L3 = 재분류 + 신설 3 종. L4 만 *진짜 새 발명*. 4 단계 모양은 산업 등급 표준 (SLSA: 4 등급, PCI DSS: 4 등급, CMM: 5 등급) 과 정합하여 *부분 호환성* 가능.
